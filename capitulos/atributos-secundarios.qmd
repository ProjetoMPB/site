...
**2H3** -- nível das funções (contém seis aspectos):
...
- 2H3b -- compara a distribuição de 12 classes (e/ou subclasses) mais significativas de categorias funcionais, a saber, DIA,  DS, SubV, DIM, D-pte, EC1, EC2).
...
- 2H3f -- compara os valores máximos e mínimos de entropia [ver capítulo Probabilidade e entropia](probabilidade.qmd) das categorias funcionais nos diferente *corpora*.
...
![Valores mínimo e máximo para entropia de categorias funcionais por *corpus* (entropia em bits entre parênteses).](../figs/atri-sec-22bis.png){width=50%}

#### Discussão

De um modo geral, os valores são próximos, variando entre 0.3--0.8 bit (para H mínima) e por volta de 4 bits (H máxima). Há maior divergência entre os repertórios em relação às categorias funcionais nesses extremos, especialmente quanto à entropia mínima, embora em cinco deles (Jazz, Choro, Chico Buarque, Caetano Veloso e Gilberto Gil) o posto seja assumido por bIII°. Considerando a entropia máxima, a diversidade é menor, convergindo para três alternativas: I (em oito dos *corpora*), VI (Choro, Samba e Ivan Lins) e, um tanto surpreendentemente, IV (apenas João Bosco e Milton Nascimento).

...
![Exemplos hipotéticos de replicações "fenotípicas".](../figs/composicao-15.png){width=40% #fig-comp15}

(6) Finalmente, considerar as c-palavras e r-palavras como unidades autônomas que, eventualmente, "evoluem" de maneiras independentes umas das outras. Tomando como metáfora os fenômenos biológicos (assim como foi feito na etapa anterior), denominamos o mapeamento das transformações nesse nível de derivações "genotípicas" (que eventualmente também indicarão replicações). A representação dessas relações se dá por intermédio de um esquema em árvore genealógica (uma para cada domínio, *c* e *r*), identificando as relações derivativas e replicativas. A [@fig-comp16] apresenta um breve exemplo das duas árvores.

![Exemplos hipotéticos de relações "genotípicas".](../figs/composicao-16.png){width=60% #fig-comp16}

---

Concluída a primeira fase (modelagem), passa-se então à segunda, a da composição assistida. O protocolo dessa fase consiste nas seguintes etapas:

(1) Gerar uma r-palavra-semente para todo o processo^[Formando uma unidade referencial que pode ser aproximadamente associada à ideia de *Grundgestalt*, de Arnold Schoenberg. Para maiores detalhes sobre o conceito, ver, entre outras fontes, @almada-mda.] por intermédio do algoritmo-compositor *corporakov_ritmo.py*. O *corpus* referencial deverá ser, evidentemente, o mesmo da obra selecionada para modelagem. O usuário pode escolher livrevemente o número de r-letras de sua palavra-semente, bem como descartar r-palavras produzidas pelo algoritmo que porventura não lhe convenham, até a obtenção de uma unidade ideal.

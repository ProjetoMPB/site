...
**Estratégia 2** -- o "problema" da ausência de controle sobre a estrutura musical em alto nível foi recentemente equacionado com a idealização da segunda estratégia de composição assistida. Podendo também ser denominada uma "modelagem derivativa-estilística", envolve, além dos processos markovianos da estratégia 1 (em menor escala, como veremos), uma concepção evolutiva-musical. Isso se dá a partir de uma adptação simplificadora do Modelo de Análise Derivativa (MDA, na sigla em inglês), adaptação essa identificada pelo acrônimo MDA*.^[Para descrições e maiores detalhes sobre MDA e MDA*, ver @almada-mda.] 

Essencialmente, a estratégia 2 é baseada na ideia de *microtransformações parcimoniosas* (MTP). Definimos uma MTP como uma espécie de "edição" aplicada a um dos componentes de uma dada c- ou  r-palavra a partir de alguma operação transformacional. Em outros termos, a MTP afeta uma r-letra ou uma c-letra isolada da estrutura referencial, sem 

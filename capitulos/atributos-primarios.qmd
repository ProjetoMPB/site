...
**1M2c** -- Quanto à cardinalidade das c/r palavras, parece haver uma convergência para um número médio de, aproximadamente, 6 articulações melódicas, confirmando nossos pressupostos, devidamente referendados por estudos sobre segmentação de melodias, entre os quais os [@snyder-music-memory].^[Apenas o *corpus* Jazz parece se distanciar desse eixo (com preferência por segmentos com quatro articulações), o que pode suscitar estudos futuros específicos.]
     ...
     \item \textbf{1M2d} e \textbf{1M2e} -- Vistos em conjunto, os dois atributos -- que, em suma, medem o grau de estabilidade melódica -- corroboram, \textit{grosso modo}, a noção geral de que existiria uma espécie de ``lei de compensação'' em linhas melódicas que, idealmente, tenderiam para uma retificação (ou seja, saltos e vales tenderiam a se anular mutuamente). Tal hipótese fica evidente em 1M2e, pois todos os repertórios apresentam índices de estabilidade compensada com valores relativamente altos e bastante próximos entre si (a notável exceção é o \textit{corpus} Milton, com valores sensivelmente mais altos para os dois índices, reforçando a ideia da estaticidade melódica na música do compositor como elemento característico). A observação de 1M2d, por outro lado, embora também com valores próximos, permite destacar os \textit{corpora} Jazz e Choro dos demais, pois apresentam iE baixos e bem semelhantes, sugerindo particularidades construtivas, talvez pela presença de um número relativamente maior de movimentos arpejados e saltos em uma única direção do que nos demais repertórios (o que também poderá ser verificado em estudos futuros específicos). 

    \item \textbf{1M3a} -- Como era esperado, a detecção do ataque mais recorrente no perfil métrico de quase todos os repertórios confirma a proeminência da posição 1 da grade. Há apenas duas exceções: (1) como era esperado, o \textit{corpus} Samba tem em destaque a posição 10 (a quarta semicolcheia do tempo), sugerindo o alto grau de sincopação do gênero. O aspecto será melhor detalhado nos atributos secundários correspondentes.\footnote{Para um detalhamento desse tópico, ver ALMADA; ZISELS (2023).} (2) no caso do \textit{corpus} Rita Lee, a proeminência da posição 7 se deve ao fato de que, em média, as linhas melódicas da compositora apresentam grande recorrência de colcheias (ver também o atributo 1M3b), associando-se a uma forte tendência para finalizações contramétricas das r-palavras (o que contribui para uma maior presença da posição 7 em relação à posição cométrica 1).

    \item \textbf{1M3b} -- Nesta abordagem primária do atributo, que evidencia apenas a r-letra mais comum, os resultados são inconclusivos, pois tendem a capturar a alternativa \textit{b} (ataque único, uma espécie de ``I grau'' rítmico), bastante comum em todos os repertórios (e é o que se observa nos resultados obtidos). Destaque-se as exceções que confirmam as células básicas dos gêneros dos \textit{corpora} de controle: \textit{n} (Samba) e \textit{v} (Choro), bem como a alta porcentagem em que se manifesta \textit{b} no Jazz (cerca do dobro em relação a outros repertórios). Registre-se também a proeminência de \textit{j} nos \textit{corpora} Chico Buarque, Caetano Veloso, Milton Nascimento, Gilberto Gil e Rita Lee (neste, o maior percentual de todos -- 42.5\% -- quase o dobro em relação à média). Resultados mais significativos sobre o aspecto em questão serão trazidos no exame dos atributos secundários correspondentes.

    \item \textbf{1M3c} -- Algo semelhante ao observado no atributo 1M2b (que mede a distribuição das c-palavras) ocorre aqui, ainda que numa relativamente menor dispersão: o grande número de r-palavras dificulta a obtenção de conclusões substanciais na comparação dos \textit{corpora}.\footnote{Registre-se, entretanto, o significativo fato de que no \textit{corpus} Djavan a r-palavra mais comum é a mesma do \textit{corpus}Samba -- nn --, o que pode denotar a forte contrametricidade do repertório do compositor, um fato observado especialmente em suas peças em gênero de samba.}

    \item \textbf{1M3c} -- Neste caso, ao contrário, a medição do índice de contrametricidade produz informações relevantes em relação à determinação estilística. É possível dividir os \textit{corpora}, nesse aspecto, em três níveis de sincopação: (1) alto (acima de 0.7), epitomada pelo \textit{corpus} Samba; (2) médio (entre 0.6 a 0.7), com vários dos repertórios analíticos (tendo o \textit{corpus} Chico Buarque, dentre esses, o maior valor relativo para ic) e (3) baixo (menor que 0.6), com os repertórios de Choro, Jazz, Caetano Veloso e Milton Nascimento (o menor ic de todos). Isso nos leva a conjecturar que tal característica abstrata do samba (levando em conta aqui apenas a sincopação rítmica, bem entendido) se infiltra, em maior ou menor medida, em alguns estilos composicionais individuais. O exame dos atributos secundários buscará validar esta hipótese.

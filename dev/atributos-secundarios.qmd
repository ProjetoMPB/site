...
```{python}
exibir_tabela_atributos("2H3c-E1", titulo="2H3c - Empréstimos cromáticos de primeira ordem", subtitulo='Distribuição das categorias funcionais da subclasse "Empréstimos cromáticos de primeira ordem" (E1)')
```
...
```{python}
exibir_tabela_atributos("2H3c-E2", titulo="2H3c - Empréstimos cromático de segunda ordem", subtitulo='Distribuição das categorias funcionais da subclasse "Empréstimos cromáticos de segunda ordem" (E2)')
```


#### Discussão

Apesar de sua relativa raridade, os empréstimos de segunda ordem contribuem ainda com mais intensidade para a marcação das fronteiras da MPB em relação ao superconjunto no qual se insere. Como mostram os dados, o *corpus* Jobim é o que mais se destaca nesse atributo, sendo o único no qual todas as categorias mapeadas estão presentes. Por outro lado, nos *corpora* João Bosco, Caetano Veloso, Rita Lee e, especialmente, Cartola as categorias da classe E2 são ainda mais raras, o que faz distanciar esse grupo daquele que podemos denominar o núcleo MPB referente a tal classe.
```{python}
